...
function PhoneNumberVerification(props) {
  ...
  const [VerifyNewNumber, { loading, data, error }] = useMutation(PHONE_NUMBER_VERIFICATION, { variables: { token: token}});

  useEffect(() => {
    VerifyNewNumber().then(res => {
      console.log(res)
    }).catch(err => {
      console.log(err)
    })
            ...
            <MotionContainer initial='initial' open>
              <Box sx={{ maxWidth: 680, margin: 'auto', textAlign: 'center' }}>
                ...
                <motion.div variants={varFadeInDown}>
                  <Paper sx={{ p: 2 }}>
                    <Typography variant='body1'>
                      Your Phone Number was not updated due to some errors, you can try again or contact our support team
                    </Typography>
                  </Paper>
                </motion.div>
