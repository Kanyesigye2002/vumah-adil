...
export default function PermissionsTable({setPermissions}) {

  const {error, data} = useQuery(GET_GROUP_ROLES)
            ...
            <TableHead>
              <TableRow>
                ...
                <TableCell align='right' sx={{ p: '8px' }}>
                  <FormControlLabel value='Read Only' control={<Radio />} label='Read Only' labelPlacement='bottom'
            ...
            <TableBody>
              {BASIC_TABLE.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell align='center'>{row.readOnly ? <Done color='success'/> : <Remove color='error'/>}</TableCell>
                  <TableCell align='center'>{row.standard ? <Done color='success'/> : <Remove color='error'/>}</TableCell>
                  <TableCell align='center'>{row.admin ? <Done color='success'/> : <Remove color='error'/>}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
