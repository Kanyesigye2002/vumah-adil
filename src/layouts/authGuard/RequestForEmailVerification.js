...
function RequestForEmailVerification(props) {
  ...
  const [CreateMailVerification, {data}] = useMutation(SEND_EMAIL_VERIFICATION, {
    variables: { userName: values.email }
  });

  return (
    <div style={{ width: '400px', maxWidth: '400px' }}>
      <Typography variant='h5' textAlign='center' sx={{ color: 'text.secondary', mb: 3, px: 2 }}>Request for a new
        verification email</Typography>
      {success ?
